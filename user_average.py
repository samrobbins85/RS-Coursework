...
def content_based(user_id, toremove):
    ...
    dataindex = data[
        (data["user_id"] == chosen_user) & (data["business_id"].isin(toremove))
    ].index
...
# Sample half of the businesses the user has reviewed to hide for evaluation
toremove = (
...
dfindex = df[(df["user_id"] == chosen_user) & (df["business_id"].isin(toremove))].index
